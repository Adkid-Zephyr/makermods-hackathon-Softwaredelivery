...
### 2026-03-31
- **docs**: Packaged the repository for public hackathon delivery. Rewrote `README.md` to reflect the full software delivery scope instead of only the UI, and to include the Hugging Face dataset/model links, delivery video entry, real project status, deployment recommendations, future improvement directions, environment-failure postmortem, Git isolation reminder, and pointers to bug/developer docs. Added `docs/BUG_LOG.md`, `docs/DEVELOPER_WARNINGS.md`, `docs/assets/makermods-delivery-video.mp4`, `docs/assets/makermods-delivery-poster.png`, and `LICENSE`.
- **cleanup**: Removed internal and draft-only materials that were not appropriate for the public delivery repo, including `CLAUDE.md`, `frontend/ideas.pen`, unused generated mockup images, and unused default scaffold assets. Added `README.zh-CN.md` and language links so the repository now has both English and Chinese README entry points. Updated `.gitignore` to ignore accidental Finder-style duplicate files.
- **docs**: Added a richer README showcase with project photos, the final delivery video, and a second ACT bread-grasp demo video produced by the trained model. Moved README-facing media under `docs/assets/photos/` and `docs/assets/videos/`.
- **docs**: Converted the ACT bread-grasp demo into an animated GIF so the README now shows motion directly instead of only a static poster for the trained-model result.
...
- **feat**: Trained policy paths are now saved to localStorage and shown as a dropdown on the inference page. Each time a training job completes, its output model ID is persisted. On the inference page, clicking the policy path field (or the chevron button) shows all previously trained policies to pick from, with the ability to delete entries. Users can still type a custom path. Modified: `frontend/components/wizard/steps/inference-step.tsx`, `frontend/components/wizard/steps/training-step.tsx`.
- **fix**: Port locks now reliably released when a subprocess crashes. Previously, if inference/recording/teleoperation failed (e.g. invalid dataset name), the serial port locks were not released, blocking subsequent attempts with "Port in use by X". The frontend polling now explicitly calls the stop endpoint when it detects process error/completion, ensuring locks are cleaned up even if the backend's automatic cleanup hasn't finished yet. Modified: `frontend/components/wizard/steps/inference-step.tsx`, `frontend/components/wizard/steps/record-step.tsx`, `frontend/components/wizard/steps/teleoperate-step.tsx`.
- **fix**: Training duration field now accepts any value greater than 0 (e.g. 0.001) instead of requiring at least 0.5. Modified: `frontend/components/wizard/steps/training-step.tsx`.
- **fix**: Recording step now waits for the process to fully exit (including HuggingFace upload) before showing the green completion checkmark. Previously, "All episodes complete" appeared as soon as episodes finished recording, misleading users into thinking the upload was done. Now shows a spinner with "Uploading to HuggingFace…" during the upload phase, and only shows the green tick after the process exits. Modified: `frontend/components/wizard/steps/record-step.tsx`.
- **fix**: Bimanual calibration path mismatch — auto-calibration was saving files to `bi_so101_follower`/`bi_so101_leader` directories, but lerobot's bimanual sub-arms (`SO101Follower`/`SO101Leader`) look in `so101_follower`/`so101_leader` (their own class name). This caused teleoperation to fail with "no motor movement detected" because lerobot couldn't find the calibration files and tried to interactively re-calibrate in a non-interactive subprocess. Fixed all calibration path references to use `so101_follower`/`so101_leader` for bimanual sub-arms. Modified: `frontend/lib/wizard-types.ts`, `backend/services/calibration_service.py`, `backend/api/calibration.py`, `frontend/lib/mock-data.ts`, `CLAUDE.md`.

### 2026-03-26
- **fix**: PortLockManager — prevents concurrent serial port access that caused backend hangs. New singleton service tracks which serial ports are in use and by what feature (teleoperation, calibration, wiggle, motor scan, etc.). Acquiring a busy port returns an immediate HTTP 409 error with a clear "Port X in use by Y" message instead of blocking forever. New file: `backend/services/port_lock_manager.py`.
- **fix**: Event loop blocking in base_control — `_send_action()` in the 50 Hz control loop now uses `asyncio.to_thread()` instead of blocking the event loop with synchronous serial I/O. Modified: `backend/services/base_control.py`.
- **fix**: Timeouts on all serial bus operations — `asyncio.wait_for()` wraps all `to_thread` motor bus calls (wiggle: 15s, motor scan: 15s, manual calibration: 5-10s) so hardware hangs don't block the backend forever. Modified: `backend/api/setup.py`, `backend/api/debug.py`, `backend/api/calibration.py`.
- **fix**: Manual calibration `write_calibration_to_motors` now reuses the existing bus connection instead of opening a fresh one on the same port. New method `write_calibration_to_bus()`. Modified: `backend/services/manual_calibration.py`, `backend/api/calibration.py`.
- **feat**: Port lock integration across all serial-port endpoints — teleoperation, recording, inference, CLI calibration, auto-calibration (subprocess mode with process→port mapping), wiggle, motor scan, manual calibration WS, base control connect/disconnect/detect. Modified: `backend/api/teleoperation.py`, `backend/api/recording.py`, `backend/api/inference.py`, `backend/api/calibration.py`, `backend/api/setup.py`, `backend/api/debug.py`, `backend/api/base_control.py`.
- **feat**: `GET /api/system/port-locks` endpoint shows current port lock status for diagnostics. Modified: `backend/api/system.py`.
- **fix**: Shutdown cleanup releases all port locks. Modified: `backend/main.py`.
- **fix**: Number input fields (episodes, episode time, reset time, duration, batch size) no longer auto-correct to a default when cleared. Instead, the field accepts empty/zero values and shows a red border with a validation warning (e.g., "Must be at least 1"). The Start/Submit button is disabled while any field is invalid. Modified: `frontend/components/wizard/steps/record-step.tsx`, `frontend/components/wizard/steps/inference-step.tsx`, `frontend/components/wizard/steps/training-step.tsx`.
- **fix**: Port locks are now automatically released when a subprocess exits naturally (e.g., auto-calibration completes, recording finishes all episodes). Previously, locks were only released via the explicit stop endpoint, causing orphaned locks that blocked all subsequent operations until server restart. Modified: `backend/services/process_manager.py` (`_collect_logs` finally block).

### 2026-03-23
- **fix**: Inference page form fields now persist across step navigation and page refresh via localStorage. Previously, all entered data (policy path, repo ID, task, etc.) was lost when switching to another wizard step. Modified: `frontend/components/wizard/wizard-provider.tsx`.
- **feat**: Evaluation Repo ID field on inference page now shows a warning when the dataset name doesn't start with "eval_", since lerobot requires this prefix for evaluation datasets. Modified: `frontend/components/wizard/steps/inference-step.tsx`.
- **feat**: Training page (Step 7) powered by Qualia Studios. Users can train ACT policies on recorded datasets via Qualia's managed GPU infrastructure. Features: API key validation and secure storage (`.env`, gitignored), HuggingFace dataset selection (auto-prefilled from recording step), model architecture selector (ACT available, SmolVLA/Pi0/Pi0.5/GR00T coming soon), read-only camera mapping display (front_cam→image_top, hand_cam→image_wrist, side_cam→image_side), GPU instance selection cards with specs and pricing, configurable batch size/duration/output model name/job description, live job status tracker with phase progression (queuing through completion), job cancellation support. Backend creates a new Qualia project per training job. New files: `backend/models/training.py`, `backend/services/qualia_service.py`, `backend/api/training.py`, `frontend/components/wizard/steps/training-step.tsx`. Modified: `backend/main.py`, `frontend/lib/wizard-types.ts`, `frontend/lib/api.ts`, `frontend/lib/services.ts`, `frontend/components/wizard/wizard-provider.tsx`, `frontend/components/wizard/wizard-layout.tsx`, `frontend/components/wizard/wizard-sidebar.tsx`.
- **feat**: Base (LeKiwi 3-wheel omni) keyboard control panel in the Teleoperation step. Users can drive the XLerobot base from the browser using keyboard keys (I/K forward/back, J/L strafe, U/O rotate, N/M speed). Auto-detects which motor controller port has base motors (IDs 7, 8, 9) with manual fallback. Uses browser keyboard events over WebSocket — no pynput or macOS Accessibility permissions needed. Backend runs a 50 Hz smooth acceleration/deceleration control loop. Teleoperation step is now split into two separate sections: "Arm Teleoperation" and "Base (Keyboard) Control" with mutual exclusion — only one can be active at a time since they share the serial port. New files: `backend/services/base_control.py`, `backend/api/base_control.py`, `backend/websockets/base_control.py`, `frontend/components/wizard/steps/base-control-panel.tsx`. Modified: `backend/main.py`, `frontend/lib/services.ts`, `frontend/components/wizard/steps/teleoperate-step.tsx`.

### 2026-03-20
- **feat**: Record step now checks HuggingFace authentication on mount via the `huggingface_hub` Python API (same method lerobot uses for uploads). If not logged in, shows a warning with `huggingface-cli login` instructions and blocks recording. If logged in, shows username and pre-fills the repo ID placeholder. Backend `hf_service.py` rewritten to use `HfApi().whoami()` and `HfApi().create_repo()` instead of shelling out to `huggingface-cli` binary (which may not be on PATH). Modified: `backend/services/hf_service.py`, `frontend/lib/services.ts`, `frontend/components/wizard/steps/record-step.tsx`.
- **fix**: (Reverted 2026-03-27) Bimanual calibration paths were incorrectly changed to `bi_so101_follower`/`bi_so101_leader` — the correct directories are `so101_follower`/`so101_leader` since the bimanual sub-arms use their own class name for calibration lookup.
